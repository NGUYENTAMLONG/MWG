...
import { IsNull, Like, Not, DataSource } from 'typeorm';
import {
  BadRequestException,
  Injectable,
  NotFoundException,
...
export class QuestionsService {
  ...
  public async createQAFromExcel(): Promise<any> {
    ...
    try {
      ...
      if (
        checkAnswers.existedCorrect &&
        !checkAnswers.flag &&
        checkAnswers.invalidQuestions.length > 0
      ) {
        throw new BadRequestException(
          EXCEPTION_QUESTION.MORE_THAN_ONE_CORRECT_ANSWER,
        );
      }
      if (
        !checkAnswers.existedCorrect &&
      ...
      for (const element of data) {
        ...
        for (const answer of answers) {
          ...
          const savedAnswer = await queryRunner.manager.save(
            AnswerEntity,
            newAnswer,
          );
        }
